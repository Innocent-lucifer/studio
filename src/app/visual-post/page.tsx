...
function VisualPostPageComponent() {
  ...
  const handleGeneration = useCallback(async (
    ...
    const isNewImage = imageUri !== generationState.current.processedImageUri;
  ...
  const commonFooter = (
      <footer className="text-center p-4 mt-12 text-slate-500 text-sm">
        <p>© {new Date().getFullYear()} SagePostAI. All rights reserved.</p>
      </footer>
  );
      ...
      <main className="container mx-auto w-full max-w-3xl">
        ...
        <Card className="bg-slate-800/60 backdrop-blur-md border border-slate-700 shadow-2xl hover:shadow-primary/20 transition-shadow duration-300 rounded-2xl p-6 sm:p-8">
          ...
          >
            ...
            >
              <Icons.trash className="mr-2 h-4 w-4" /> Start Over
            </Button>
          </motion.div>
          <HiddenFileInput />
              ...
              <div className="flex flex-wrap gap-3">
                {toneOptions.map(({ label, value, icon }) => {
                  ...
                  <Button
                    ...
                    disabled={(!userIdToPass && value !== 'default') || isLoading} 
                    className={`
                      ${selectedTone === value
              ...
              {!isLoading && !error && !generatedPost && (
                 <div className="flex flex-col items-center justify-center p-6 rounded-lg bg-slate-700/50 min-h-[150px] text-slate-400 text-center">
                  {!userIdToPass ? (
                    ...
                    <>
                      <Icons.info className="h-8 w-8 mb-3"/>
                      <span className="text-lg">Your post will appear here once generated.</span>
                    </>
                  )}
                </div>
              )}
            ...
            {!isLoading && generatedPost && (
              <motion.div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:gap-3 mt-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{delay: 0.4}}>
                <Button 
                  ...
                  disabled={!userIdToPass || isLoading}
                ...
                <Button 
                  ...
                  disabled={isSavingDraft || !userIdToPass || isLoading}
             ...
             {!userIdToPass && !isLoading && imageDataUri &&
